...
def handle_sp(message):
    ...
    # --- Step 4: Send "checking..." message ---
    checking_msg = checking_status_format(cc, "Stripe Premium Auth", bin_info)
    status_message = bot.reply_to(message, checking_msg, parse_mode='HTML')

    # --- Step 5: Run SP check ---
    start_time = time.time()
    check_result = process_card_sp(cc)  # This calls the function from sp.py
    end_time = time.time()
    time_taken = round(end_time - start_time, 2)
...
def handle_msp(message):
    ...
    try:
        ...
        initial_msg = f"🚀 Starting mass Stripe Premium Auth check of {len(cards)} cards..."
        status_message = bot.reply_to(message, initial_msg)
